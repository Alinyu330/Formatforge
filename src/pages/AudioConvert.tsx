  ...
  return (
    <ConvertLayout>
      <div className="space-y-4 sm:space-y-5">
        <div>
          <h2 className="text-base sm:text-lg font-semibold text-white/85">音频格式转换</h2>
          <p className="text-[10px] sm:text-xs text-white/35 mt-1">标准格式及 QQ音乐(QMC/MFLAC)、网易云(NCM)、酷狗(KGM) 加密格式解密转换</p>
        </div>

        <FileUpload type="audio" onFilesAdd={(f) => addFiles(f, 'audio')}
          accept="audio/*,.mp3,.flac,.wav,.aac,.ogg,.m4a,.wma,.opus,.ape,.wv,.aiff,.alac,.qmc0,.qmc3,.qmc4,.qmc6,.qmc8,.qmcflac,.mgg,.mgg1,.mflac,.mflac0,.ncm,.kgm,.kgma,.kwm,.vpr"
        ...
        {tasks.length > 0 && (
          <>
            <div>
              ...
              <FormatMultiSelector formats={AUDIO_FORMATS} selected={selectedFormats} onChange={applyFormats} />
            </div>
            <div>
              <p className="text-[10px] uppercase tracking-wider text-white/30 mb-2">转换参数</p>
              <AudioOptions />
            </div>
          </>
        )}
          ...
          <div className="flex flex-wrap items-center gap-2 sm:gap-3">
            <button onClick={startConversion} disabled={isProcessing || pendingCount === 0}
              className="flex items-center gap-1.5 sm:gap-2 px-4 sm:px-5 py-2 sm:py-2.5 rounded-xl text-xs sm:text-sm font-medium
                bg-[#00d4ff] text-[#0f1724] hover:bg-[#00d4ff]/90
                disabled:opacity-30 disabled:cursor-not-allowed transition-all shadow-[0_0_25px_rgba(0,212,255,0.2)]">
            ...
            {doneCount > 0 && (
              <button onClick={downloadAllAsZip}
                className="flex items-center gap-1.5 sm:gap-2 px-4 sm:px-5 py-2 sm:py-2.5 rounded-xl text-xs sm:text-sm font-medium
                  border border-[#00d4ff]/30 text-[#00d4ff] hover:bg-[#00d4ff]/10 transition-all">
