...
export default function VideoConvert() {
  ...
  return <ConvertLayout>
    <div className="space-y-4 sm:space-y-5">
      ...
      <FileUpload type="video" onFilesAdd={files => addFiles(files, 'video')} accept={VIDEO_ACCEPT} />
      <VideoOptions />
      <ConvertQueue />
      {tasks.length > 0 && <div className="flex flex-wrap items-center gap-2 sm:gap-3"><button type="button" onClick={startConversion} onTouchEnd={(e) => { e.preventDefault(); startConversion(); }} disabled={pendingCount === 0} className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-medium bg-[#00d4ff] text-[#0f1724] disabled:opacity-30"><Play className="w-4 h-4" fill="currentColor" />{convertingCount > 0 ? '转换中...' : pendingCount === 0 ? (doneCount > 0 ? '转换完成' : '请先选择目标格式') : `开始转换 (${pendingCount})`}</button>{doneCount > 0 && <button onClick={downloadAllAsZip} className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-medium border border-[#00d4ff]/30 text-[#00d4ff]"><Archive className="w-4 h-4" />打包下载全部 ({doneCount})</button>}<button onClick={clearTasks} className="px-3 py-2.5 text-xs text-[var(--text-muted)]">清空列表</button></div>}
    </div>
  </ConvertLayout>;
}