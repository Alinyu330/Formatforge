...
import { isQMCFile, decryptQMC, decryptMusicexWithEkey, fetchEkeyFromAPI, isMusicexFormat, parseMusicexFooter, parseStagMeta, decryptStagWithEkeyCandidates } from './qmc';
...
async function convertNative(
  ...
  if (fileType === 'audio') {
    ...
    if (isQMCFile(task.fileName)) {
      let result;
      const stagMeta = parseStagMeta(raw);
      if (stagMeta) {
        ...
        result = await decryptStagWithEkeyCandidates(raw, cred, stagMeta, task.sourceFormat, '27');
      } else if (isMusicexFormat(raw)) {
