// 酷狗音乐 KGM/KGMA/VPR 加密格式解密
// 参考 unlock-music 的 KGM 解密算法（V2 mask）

// 16 字节文件头魔数：KGM 与 VPR 两种变体
const KGM_HEADER = new Uint8Array([
  0x7c, 0xd5, 0x32, 0xeb, 0x86, 0x02, 0x7f, 0x4b,
  0xa8, 0xaf, 0xa6, 0x8e, 0x0f, 0xff, 0x99, 0x14,
]);

const VPR_HEADER = new Uint8Array([
  0x05, 0x28, 0xbc, 0x96, 0xe9, 0xe4, 0x5a, 0x43,
  0x91, 0xaa, 0xbd, 0xd0, 0x7a, 0xf5, 0x36, 0x31,
]);

// VPR 变体额外需要异或的掩码（17 字节循环）
const VPR_MASK_DIFF = new Uint8Array([
  0x25, 0xdf, 0xe8, 0xa6, 0x75, 0x1e, 0x75, 0x0e,
  0x2f, 0x80, 0xf3, 0x2d, 0xb8, 0xb6, 0xe3, 0x11, 0x00,
]);

const TABLE_1 = new Uint8Array([
  ,
]);

const TABLE_2 = new Uint8Array([
  ,
]);

const MASK_V2_PRE_DEF = new Uint8Array([
  ,
]);

function matchesHeader(data: Uint8Array, header: Uint8Array): boolean {
  if (data.length < header.length) return false;
  for (let i = 0; i < header.length; i++) {
    if (data[i] !== header[i]) return false;
  }
  return true;
}

function getMask(pos: number): number {
  let offset = pos >> 4;
  let value = 0;
  while (offset >= 0x11) {
    value ^= TABLE_1[offset % 272];
    offset >>= 4;
    value ^= TABLE_2[offset % 272];
    offset >>= 4;
  }
  return MASK_V2_PRE_DEF[pos % 272] ^ value;
}

function detectFormat(data: Uint8Array): string {
  if (data.length < 4) return 'mp3';
  if (data[0] === 0x66 && data[1] === 0x4c && data[2] === 0x61 && data[3] === 0x43) return 'flac';
  if (data[0] === 0x4f && data[1] === 0x67 && data[2] === 0x67 && data[3] === 0x53) return 'ogg';
  if (data[0] === 0x52 && data[1] === 0x49 && data[2] === 0x46 && data[3] === 0x46) return 'wav';
  if ((data[0] === 0xff && (data[1] & 0xe0) === 0xe0) || (data[0] === 0x49 && data[1] === 0x44 && data[2] === 0x33)) return 'mp3';
  if (data[0] === 0xff && (data[1] & 0xf6) === 0xf0) return 'aac';
  return 'mp3';
}

export function decryptKGM(data: Uint8Array): { data: Uint8Array; ext: string } {
  const isVpr = matchesHeader(data, VPR_HEADER);
  if (!isVpr && !matchesHeader(data, KGM_HEADER)) {
    throw new Error('不是有效的 KGM/KGMA/VPR 文件（文件头不匹配）');
  }
  if (data.length < 0x2c) {
    throw new Error('KGM 文件头不完整');
  }

  // 0x10 处为音频数据起始偏移（小端 uint32）
  const headerLen = (data[0x10] | (data[0x11] << 8) | (data[0x12] << 16) | (data[0x13] << 24)) >>> 0;
  if (headerLen < 0x2c || headerLen > data.length) {
    throw new Error('KGM 文件头长度异常');
  }

  // 文件密钥位于 0x1c..0x2c，共 16 字节，第 17 字节固定为 0
  const key = new Uint8Array(17);
  key.set(data.subarray(0x1c, 0x2c), 0);

  const src = data.subarray(headerLen);
  const out = new Uint8Array(src.length);
  for (let i = 0; i < src.length; i++) {
    let med8 = key[i % 17] ^ src[i];
    med8 ^= (med8 & 0x0f) << 4;
    let msk8 = getMask(i);
    msk8 ^= (msk8 & 0x0f) << 4;
    out[i] = med8 ^ msk8;
    if (isVpr) out[i] ^= VPR_MASK_DIFF[i % 17];
  }

  return { data: out, ext: detectFormat(out) };
}

export function isKGMFile(filename: string): boolean {
  const ext = filename.split('.').pop()?.toLowerCase() || '';
  return ['kgm', 'kgma', 'vpr'].includes(ext);
}

/** 验证文件头是否为合法的 KGM/KGMA/VPR 加密文件 */
export function isValidKGMHeader(bytes: Uint8Array): boolean {
  return matchesHeader(bytes, KGM_HEADER) || matchesHeader(bytes, VPR_HEADER);
}