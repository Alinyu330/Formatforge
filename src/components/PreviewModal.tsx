...
export default function PreviewModal({ taskId, onClose }: Props) {
  ...
  function getPreviewType(): 'audio' | 'image' | 'sheet' | 'document' | 'unknown' {
    const ext = item.targetFormat || task!.sourceFormat;
    if (['mp3', 'flac', 'wav', 'aac', 'ogg', 'm4a', 'wma'].includes(ext) ||
        ['mp3', 'flac', 'wav', 'aac', 'ogg', 'm4a', 'wma'].includes(task!.sourceFormat)) {
      return 'audio';
    }
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm"
      ...
      <div className="bg-[#111827] border border-white/[0.08] rounded-xl sm:rounded-2xl w-full max-w-3xl max-h-[90vh] sm:max-h-[85vh] flex flex-col shadow-2xl overflow-hidden mx-2 sm:mx-0">
        {/* Header */}
        <div className="flex items-center gap-2 sm:gap-3 px-3 sm:px-5 py-3 sm:py-4 border-b border-white/[0.06]">
          <span className="text-xs sm:text-sm text-white/80 truncate flex-1">{task.fileName}</span>
          <span className="text-[9px] sm:text-[10px] text-[#00d4ff] uppercase">{task.targetFormat}</span>
          <button onClick={handleDownload} className="p-1.5 sm:p-2 rounded-lg hover:bg-white/[0.06] text-white/40 hover:text-white/70 transition-colors" title="下载">
            <Download className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
          </button>
          <button onClick={onClose} className="p-1.5 sm:p-2 rounded-lg hover:bg-white/[0.06] text-white/40 hover:text-white/70 transition-colors">
          ...
          {type === 'audio' && (
            <div className="flex flex-col items-center justify-center gap-6 py-12">
              <div className={`w-24 h-24 rounded-full flex items-center justify-center transition-all duration-500
                ${isPlaying ? 'bg-[#00d4ff]/20 shadow-[0_0_40px_rgba(0,212,255,0.2)] scale-110' : 'bg-white/[0.04]'}`}>
                <div className={`w-12 h-12 rounded-full flex items-center justify-center
                  ${isPlaying ? 'bg-[#00d4ff]/30' : 'bg-white/[0.06]'}`}>
                  {isPlaying ? (
              ...
              <p className="text-sm text-white/60">{task.fileName}</p>
              <button
                onClick={() => {
              ...
              <audio
                ...
                className="hidden"
              />
              <audio src={task.resultUrl || ''} controls className="w-full max-w-md mt-4" />
            </div>
          )}
                ...
                <div
                  ...
                  dangerouslySetInnerHTML={{
                    __html: type === 'sheet' ? previewContent :
                            ext === 'txt' ? `<pre style="white-space:pre-wrap;font-family:monospace;font-size:13px;line-height:1.7;color:#ccc;">${escapeHtml(previewContent)}</pre>` :
                            previewContent
                  }}
                />
              )}
