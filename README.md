...
1. **Windows 客户端（推荐，直接下载）**：👉 [点击下载 FormatForge-Setup-1.3.11.exe](https://dl.formatforge.asia/FormatForge-Setup-1.3.11.exe?v=20260914v30)

   可复制链接（国内 CDN 加速）：

   ```
   https://dl.formatforge.asia/FormatForge-Setup-1.3.11.exe?v=20260914v30
   ```

   备用链接（GitHub）：

   ```
   https://github.com/Alinyu330/Formatforge/releases/download/backup-20260914-v30/FormatForge-Setup-1.3.11.exe
   ```
...
1. **直接下载安装客户端（APK，直接下载，国内推荐）**：👉 [点击下载 FormatForge-v30.apk](https://formatforge.asia/Formatforge/FormatForge-v30.apk)

   可复制链接（国内主链接，加载快）：

   ```
   https://formatforge.asia/Formatforge/FormatForge-v30.apk
   ```

   备用链接（GitHub，国内网络可能无法访问）：

   ```
   https://github.com/Alinyu330/Formatforge/releases/download/backup-20260914-v30/FormatForge-v30.apk
   ```

   手机直接点击下载；下载后点击安装，需允许「安装未知来源应用」；同签名版本可直接覆盖安装升级
   > v21 起客户端支持**应用内更新**：启动自动检查新版本，确认后自动下载并唤起系统安装器，是否安装由你确认。v25 起客户端「使用说明」改为应用内全屏打开（优先在线版、离线回退本地副本，顶部一键返回主页）；v26 起「历史版本」页面也在应用内打开（不再跳转浏览器，可离线查看）。v20 及更早版本需手动下载安装本次更新。
...
- **新增**：QQ 音乐新版数字尾数加密格式（mflac2 / mgg2 / mflac0 等，mqms2 家族）解密转换支持——手机下载后传输到电脑的文件，可在网页 / Windows 端正常解密转换
- **修复**：此前该类文件「转换成功」但输出为几 KB 损坏音频、无法播放的问题（旧代码不认识新文件布局）
- **说明**：自动解析文件内嵌歌曲信息拉取解密密钥（ekey），多候选重试并以音频完整性校验兜底；个别文件因 QQ 音乐服务端资源已下架而无法解密（已知限制）
...
- **改进**：客户端构建流程——打包 Windows 客户端时**自动同步最新页面资源**，从根源杜绝「功能已修复但安装包内仍是旧代码」的问题再次发生
- **说明**：Android 客户端同步升级至 v27（功能与 v26 一致），保持三端版本对齐
...
- **改进**：客户端「历史版本」改为**应用内打开**（不再跳转系统浏览器）——站内原生页面即时加载、离线可用，顶部「返回首页」一键回到客户端主页
- **改进**：网页端使用说明「返回」按钮回到**打开说明时所在的页面**（如首页 / 历史版本页），不再跳到工具主页；新窗口打开时优先自动关闭窗口返回原页
...
- **改进**：客户端「使用说明」改为**应用内全屏打开**（不再跳转系统浏览器）——优先加载在线版内容保持实时更新，离线自动回退本地副本，顶部「返回主页」一键回到客户端主页
- **调整**：「检查更新」按钮移至首页**顶部右侧**（与主题切换并列），远离底部反馈邮箱，移动端不再误触
- **性能**：视频转换提速（三端同步生效）——Android 客户端补齐 x264 编码速度档位（约快 3–4 倍）、WebM 编码启用实时模式（约快 5–10 倍）、缩放滤镜换用更快的算法
- **修复**：网页端打开使用说明偶发黑屏（Service Worker 导航劫持 `.html` 文档，denylist 正则已兼容查询串并彻底放行）
